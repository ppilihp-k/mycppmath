...
{
	if(s_terminateAllocator = 0xFFFFFFFF)
	{
		s_terminateAllocator = 0x0;
		s_allocator = new MyAllocator();		
	}	
};

void Vector4f::terminateSmartAllocator()
{
	if(s_terminateAllocator == 0x0)
	{
		s_terminateAllocator = 0xFFFFFFFF;
		s_allocator->terminate();
		s_allocator->~MyAllocator();
		delete s_allocator;
	}
...
Vector4f::Vector4f(const Vector4f &v)
{
	...
	m_dimension = 3;
	copyVector4f(m_content,v.m_content);
};
